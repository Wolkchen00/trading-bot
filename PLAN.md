...
## v4.14 ,  R4 gözlemlenebilirlik: günlük giriş hunisi + NO_TRADE alarmı (2026-07-30, İhsan: "kaldığımız yerden devam, bitince raporla")

**Amaç:** KÖK-5'in son parçası ,  "neden işlem yok" sorusu artık logda kaybolmaz.
Strateji davranışına SIFIR etki; tüm eklemeler telemetri (hata asla döngüye sızmaz).

- **`core/funnel.py` (yeni):** ET-günü bazlı sayaçlar ,  taranan → BUY/SELL/HOLD
  sinyali → düşük güven → sektör bloku → gate bloku (sebep kırılımı) → pullback
  kuyruğu → gerçekleşen giriş/çıkış. Restart-güvenli (`state/funnel.json`,
  atomik tmp+replace, 60s yazım gazı, 30 gün saklama).
- **Gün devri raporu:** `_daily_reset` kapanan günün hunisini INFO log + kritik
  olmayan bildirimle döker (`notify_funnel_summary`).
- **NO_TRADE alarmı:** son strateji girişinden bu yana iş günü ≥
  `no_trade_alert_business_days` (3) → `notify_critical("NO_TRADE")` →
  alarms.jsonl → VPS köprüsü → ntfy. Günde en fazla 1 (dedup işareti teslimden
  ÖNCE kalıcı ,  crash'te bile en-fazla-bir; dayanıklı kuyruk kaybı önler).
  Mesajda hesap modu + gün sayısı + baskın huni aşaması + en sık gate sebebi.
- **Giriş tanımı:** yalnız strateji girişleri (BUY/SHORT tam eşleşme).
  BEAR_BUY (ters-ETF hedge) giriş SAYILMAZ ,  canlı sayım ve tarihçe
  migration'ı tutarlı (Level 10 bulgusu, fix turunda kapatıldı).
- **İş günü tanımı:** Pzt-Cum; borsa tatilleri bilinçli yaklaşıklık (kodda not).
- **Test:** +12 Codex (`test_r4_observability.py`) + 9 Claude düşmanca
  (`test_r4_adversarial.py`: patlayan telemetri akışı kırmaz, bozuk tarih,
  patlayan notifier, hafta sonu devri, eşitlikte öncelik, gaz kelebeği).
  Pytest 90/90; tam sistem 115/115.
- **Flaky test onarımı (önceden var olan, R4 dışı):** `test_v4111_bear_cycle_
  error_visibility` `>` yerine `>=` + bayat-ts-değişti; Windows saat tıkı
  kabalığında (ölçülen fark ~500µs) yanlış kırmızı üretiyordu ,  üretim
  kodu sağlam, iki kez izole repro ile kanıtlandı.
