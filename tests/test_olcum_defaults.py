...
def test_header_projection_and_tempo_warning(capsys):
    ...
    assert "Olcum donemi: 2026-07-30 -> 2026-08-09 (gun=7/30, n=1/20)" in output
    assert "Tempo projeksiyonu: 30 gun sonunda n=4.3" in output
...
def test_mandatory_post_telemetry_bar_without_event_is_integrity_fail(capsys):
    ...
    assert "[FAIL] Metrik-2 veri bütünlüğü" in output
...
def test_authoritative_state_missing_source_makes_metric4_unknown(tmp_path, capsys):
    ...
    output = capsys.readouterr().out
    assert "[FAIL] 4 kayıt/stop bütünlüğü: UNKNOWN" in output
    assert "GENEL: FAIL" in output
...
def test_empty_telemetry_has_no_completeness_miss_and_metric4_is_unknown(
    ...
    assert "[FAIL] 4 kayıt/stop bütünlüğü: UNKNOWN" in capsys.readouterr().out
...
def test_main_clients_are_mocked_and_no_order_mutation_api_is_used(
    ...
    class Data:
        def get_stock_bars(self, _request):
            raise AssertionError("no closed trade means no bar request")
    ...
    output = capsys.readouterr().out
    assert "Olcum donemi: 2026-07-30" in output
    assert "[PASS] 4 kayıt/stop bütünlüğü" in output
    assert "otoriter kaynak=broker closed-orders + persistent state + backfill" in output
...
def test_broker_closed_orders_failure_makes_metric4_unknown(
    ...
    assert "[FAIL] 4 kayıt/stop bütünlüğü: UNKNOWN" in captured.out
